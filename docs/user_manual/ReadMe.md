...
<p align="justify">Необходимо вызвать окно <b>«Привязка»</b> из меню <b>EPlaner → Привязка</b> (<a href="BindingView.md">документация по окну</a>). Если включен режим редактирования (<em>смотри предыдущий подпункт</em>), окно само переключается на дерево с галочками при выборе узла.</p>
...
<p align="justify">В этом окне можно графическим образом (<i>поставить галочки</i>) отметить, какие устройств или параметры должны быть заданы в поле. Если окно <b>«Привязка»</b> пустое, значит графическим образом добавлять данные не предусмотрено либо это другие данные, которые таким способом добавлять нельзя.</p>
