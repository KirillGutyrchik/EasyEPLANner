...
#### Окно «Устройства» (новое) [<sup>[DevicesView.md]</sup>](DevicesView.md)
...
+ [Экспорт объектов из редактора и обратный импорт](#74-Экспорт-объектов-из-редактора-и-обратный-импорт)
    ...
    + [Перенос привязанных агрегатов](#743-перенос-привязанных-агрегатов)
...
<p align="justify">Новое окно <b>«Устройства»</b> вызывается через пункт меню <b>EPlaner → Устройства (new)</b>. Подробное описание — в <a href="DevicesView.md">документации по новому окну устройств</a>. Старое окно доступно через <b>EPlaner → Устройства</b>.</p>
...
**Примечание**: 
1. Поле <code>Привязанные агрегаты</code> переносится автоматически, если в проекте-получателе найдены объекты с тем же базовым объектом и технологическим номером (<i>см. пункт <b>7.4.3</b></i>). Если подходящий объект не найден, соответствующая привязка не восстанавливается;
...
#### 7.4.3 Перенос привязанных агрегатов ####

<p align="justify">Поле <code>Привязанные агрегаты</code> в редакторе хранит глобальные номера объектов проекта. При переносе объекта в другой проект эти номера меняются, и привязки теряются.</p>

<p align="justify">При экспорте и импорте объектов привязанные агрегаты сохраняются в переносимом виде — по базовому объекту и технологическому номеру. При импорте система ищет соответствующие объекты в проекте-получателе и восстанавливает привязки. Если привязанный агрегат импортируется вместе с объектом, используется именно он, а не уже существующий в проекте объект с тем же номером. В обычном сохранении проекта используются глобальные номера; переносимый формат применяется только при экспорте и импорте объектов через редактор.</p>

**Ограничения:**
* Восстановление касается только поля <code>Привязанные агрегаты</code>. Группы танков и привязки в шагах при переносе между проектами не пересчитываются;
* Привязка восстанавливается только если в проекте-получателе уже есть объект с совпадающим базовым объектом и технологическим номером;
* Если связанный объект отсутствует в целевом проекте, соответствующая привязка не восстанавливается.

<p align="justify"><b>Рекомендация:</b> перед импортом убедитесь, что объекты, к которым настроены привязки, уже существуют в проекте-получателе, либо импортируйте их вместе.</p>
...
<p align="justify">При экспорте и импорте объектов между проектами привязки переносятся автоматически (<i>см. пункт <b>7.4.3</b></i>).</p>
