...
+ [Объекты технологической схемы](#5-Объекты-технологической-схемы)
    + [Операции объектов](#51-Операции-объектов)
        + [Модель состояний](#511-модель-состояний)
        + [Работа с состояниями в редакторе](#512-работа-с-состояниями-в-редакторе)
...
#### Окно «Привязка» [<sup>[BindingView.md]</sup>](BindingView.md)
...
<p align="justify">Окно <b>«Привязка»</b> вызывается через пункт меню <b>EPlaner → Привязка</b>. Подробное описание — в <a href="BindingView.md">документации по окну привязки</a>. Привязка каналов также доступна в окне <a href="DevicesView.md">«Устройства»</a>.</p>
...
Операции состоят из шагов, которые описываются внутри **состояний** операции.

#### 5.1.1 Модель состояний ####

Операция может находиться в одном из следующих состояний:

| Состояние в редакторе | Lua-имя | Назначение |
| --- | --- | --- |
| **Простой** | `IDLE` | операция не выполняется |
| **Запускается** | `STARTING` | переход к выполнению |
| **Выполнение** | `RUN` | основная работа операции |
| **Завершение** | `COMPLETING` | переход к успешному окончанию |
| **Выполнено** | `COMPLETE` | операция успешно завершена |
| **Становится в паузу** | `PAUSING` | переход в паузу |
| **Пауза** | `PAUSE` | операция приостановлена |
| **Выходит из паузы** | `UNPAUSING` | переход из паузы к выполнению |
| **Остановка** | `STOPPING` | переход к остановленному состоянию |
| **Остановлен** | `STOP` | операция остановлена |

Допустимые переходы между состояниями:

| Из состояния | Можно перейти в |
| --- | --- |
| Простой | Выполнение |
| Запускается | Выполнение, Остановлен |
| Выполнение | Выполнено, Пауза, Остановлен |
| Завершение | Выполнено, Остановлен |
| Выполнено | Простой |
| Становится в паузу | Пауза, Остановлен |
| Пауза | Выполнение, Остановлен |
| Выходит из паузы | Выполнение, Остановлен |
| Остановка | Остановлен |
| Остановлен | Простой |

<p align="center"><img src="images/states_transition.svg"></p>

<p align="center"><b>Рисунок</b> - <i>Переход между состояниями операций</i></p>

#### 5.1.2 Работа с состояниями в редакторе ####

В дереве технологического объекта отображаются **только заполненные** состояния (есть шаги или действия). Пустые состояния скрыты, чтобы не загромождать операцию.

Чтобы добавить состояние (или шаг в нужное состояние):
1. Выберите операцию в дереве.
2. Нажмите <kbd>Insert</kbd> или пункт контекстного меню **Создать**.
3. В окне **«Добавить состояние операции»** выберите состояние.
4. Подтвердите выбор кнопкой **Добавить**, двойным щелчком или клавишей <kbd>Enter</kbd>.

В списке окна:
- у каждого состояния своя иконка;
- уже заполненные состояния отображаются чёрным цветом;
- пустые (ещё не используемые) — серым.

После выбора в выбранное состояние добавляется шаг, и оно появляется в дереве операции. Дополнительные шаги внутри уже видимого состояния добавляются так же, как раньше: <kbd>Insert</kbd> на самом состоянии.

Рядом с названием состояния в скобках показывается число дополнительных шагов, например `Выполнение(3)`.

#### 5.1.3 Запуск при старте программы ####
...
<p align="justify">Необходимо вызвать окно <b>«Привязка»</b> из меню <b>EPlaner → Привязка</b> (<a href="BindingView.md">документация по окну</a>). Если включен режим редактирования (<em>смотри предыдущий подпункт</em>), окно само переключается в режим помощника привязки: дерево с данными для заполнения выбранной строки.</p>
...
10. **Устройства**. Служит для упрощения типовой логики работы. Устройства, которые находятся в поле *"активируемые устройства"* активируются, в поле *"деактивируемые устройства"* — деактивируются, а находящиеся в поле *"реверсные устройства"* — включаются в реверс (аналогично действию **Включать реверс**) при активном (хотя бы одном) сигнале из поля *"DI"*. Если нет ни одного сигнала из поля *"DI"*, то устройства в поле *"активируемые устройства"* деактивируются, *"деактивируемые устройства"* — активируются. Сигналы для поля *"DO"* выдаются всегда. По завершению действия все активированные устройства деактивируются, а деактивированные — активируются. Также поле *"Производительность"* служит для задания производительности активируемых моторов. Это может быть устройство **AI** (аналоговый сигнал) или номер параметра. 
11. **Группы DI DI ... -> DO DO ...**. Специальное действие - выдача дискретных сигналов при наличии хотя бы одного или всех из входных дискретных сигналов  (поле `Логика` устанавливает тип логики между входными сигналами).
12. **Группы инвертированный DI DI ... -> DO DO ...**. Специальное действие - выдача дискретных сигналов при пропадании хотя бы одного или всех из входных дискретных сигналов (поле `Логика` устанавливает тип логики между входными сигналами).
...
16. **Переход к состоянию по условию**. Аналогично предыдущему действию для перехода между состояниями согласно схеме в пункте [5.1.1](#511-модель-состояний). Условие задаёт целевое состояние из модели состояний операции.
...
8. Устройства
    * DI - **DI**, **SB**, **GS**, **FS**, **LS**;
    * DO - **DO**;
    * Активируемые устройства - **DO**, **M**, **V**, **VC**, **AO**, **ПИД-регулятор** (_вариации С_);
    * Деактивируемые устройства - **DO**, **M**, **V**, **VC**, **AO**, **ПИД-регулятор** (_вариации С_);
...
<p align="justify">Привязка агрегатов к аппаратам (<i>хотя, линии у нас относятся к агрегатам, но мы можем привязывать к ним другие агрегаты</i>) теперь выполняется не вручную, а может быть настроена также, как и ограничения (<i>окно «Привязка» в режиме редактирования</i>, см. <a href="BindingView.md">документацию</a>). Пример настройки привязки объектов представлен ниже.</p>
...
<p align="justify">В <code>Операции</code> добавлено ещё один раскрывающийся элемент <code>Дополнительные cвойства (<i>Доп. свойства</i>)</code>. В зависимости от выбранной операции, появляется разное количество полей, которое необходимо заполнить. Если в операции нет доп. свойств, то это поле не будет показано (<i>будет скрыто</i>) Заполнять можно вручную или при помощи клика по добавляемому устройству на <b>ФСА</b>. Также доступно редактирование через окно <b>«Привязка»</b> (<a href="BindingView.md">документация</a>).</p>

<p align="center"><img src="images/base_operation_props_set_up_parameters.png"></p>

<p align="center"><b>Рисунок</b> - <i>Настройка свойства через окно «Привязка»</i></p>

<p align="justify">В этом окне можно графическим образом отметить, какие устройства или параметры должны быть заданы в поле. Если окно <b>«Привязка»</b> пустое, значит графическим образом добавлять данные не предусмотрено либо это другие данные, которые таким способом добавлять нельзя.</p>
...
-- Существующие состояния: IDLE (Простой), STARTING (Запускается), RUN (Выполнение),
-- COMPLETING (Завершение), COMPLETE (Выполнено), PAUSING (Становится в паузу),
-- PAUSE (Пауза), UNPAUSING (Выходит из паузы), STOPPING (Остановка), STOP (Остановлен).
